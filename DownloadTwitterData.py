...
# for tweet in tweepy.Cursor(api.search,q=keyword, lang="en", since_id="2019-01-11").items(10):
...
dates = []
tweet_s = []
def getTimeline(limit=5000000, resultType="recent"):
        print "get tweets"
        try:
            tweets = {}
            i=0
            ...
            for tweet in tweepy.Cursor(api.search,q = keyword,
                ...
                # print"hello"
                tweet_s.append([])
                tweet_s[i].append(tweet.created_at.date())
                tweet_s[i].append(tweet.text.encode('utf-8'))
                i=i+1
                # tweet_s.append(tweet.text.encode('utf-8'))
                if tweet.created_at.date() not in tweets:
                    print "if"
                    tweets[tweet.created_at.date()] = tweet.text.encode('utf-8')
                # elif type(tweets[tweet.created_at]) == list:
                #     tweets[tweet.created_at].append(tweet.text.encode('utf-8')])
                else:
                    tweets[tweet.created_at.date()] = [tweets[tweet.created_at.date()], tweet.text.encode('utf-8')]
                    # tweets[tweet.created_at].append(tweet.text.encode('utf-8'))
...
# csvFile = open('Data/Tweets.txt', 'r')
# tweets={}
...
# # tweet_s=[]
# tweets=tweepy.Cursor(api.search,q=keyword, since="2018-2-23",lang="en").items(10)    
# # list(tweet_s)
# tweet_s = []
...
# tweets=tweepy.Cursor(api.search,q=keyword, lang="en", since_id="2018-2-23").items()

print "Fetch twitter data for "+ response+" company keyword...."

# twitterData = get_twitter_data.TwitterData('2016-04-18')
        # twitterData = get_twitter_data.TwitterData('2019-01-11')

# print 'twitterData ',type(twitterData)
...
# start = datetime.datetime(2016,04,01)
start = datetime.datetime(2019,01,11)

# end = datetime.datetime(2016,04,17)
apple = web.DataReader(keyword2, 'yahoo', start)

dates =[]
for x in range(len(apple)):
    newdate = str(apple.index[x])
...
for i in range(0,len(apple)):
    date = apple['dates'][i].replace(",","")
    yahoo_open_price.update({date:apple['Open'][i]})
    yahoo_close_price.update({date:apple['Close'][i]})
    yahoo_high_price.update({date:apple['High'][i]})
    yahoo_low_price.update({date:apple['Low'][i]})

# print type(yahoo_open_price)
...
def sentiment_analyzer():
    tokens = [tokenize(t[1]) for t in tweet_s]  # Tokenize all the tweets
    ...
    for i in range(len(afinn_total)):
        # print "tweet 0= ",tweet_s[i][0]
        # print "tweet 1= ",tweet_s[i][1]
        # print "tweet 1= ",tweet_s[i].encode('utf-8').split("|")[1]
        if afinn_total[i] > 0:
            positive_tweet_counter.append(afinn_total[i])
            csvWriter.writerow(["|positive|", tweet_s[i][0], tweet_s[i][1], float(afinn_total[i])])
        elif afinn_total[i] < 0:
            negative_tweet_counter.append(afinn_total[i])
            csvWriter.writerow(["|negative|", tweet_s[i][0], tweet_s[i][1], float(afinn_total[i])])
        else:
            neutral_tweet_counter.append(afinn_total[i])
            csvWriter.writerow(["|neutral|", tweet_s[i][0], tweet_s[i][1], float(afinn_total[i])])
    ...
    for row in list_tweet:
        ...
        if(temp_date == dateVal):
            ...
            if (sentiment == '|positive|'):
                ...
                date_NutCount+=1
    date_p.append(date_PosCount)
    date_n.append(date_NegCount)
    date_t.append(date_totalCount)
    date_nu.append(date_NutCount)
...
# # # means_tot=[]
# # # for datep in date_p:
# # #     means_men.append(datep )

# # # std_men = (2, 3, 4, 1, 2)

# # # for datep in date_n:
# # #     means_women.append(datep )

# # # std_women = (3, 5, 2, 3, 3)
# # # print std_women

# # # for datep in date_nu:
# # #     means_nut.append(datep )
...
# # # rects1 = ax.bar(index, tuple(means_men), bar_width,
# # #                 alpha=opacity, color='b',
# # #                 yerr=std_men, error_kw=error_config,
# # #                 label='Positive')

# # # rects2 = ax.bar(index+ bar_width, tuple(means_women), bar_width,
# # #                 alpha=opacity, color='b',
# # #                 yerr=std_men, error_kw=error_config,
# # #                 label='Negative')
# # # rects3 = ax.bar(index+ 2*bar_width, tuple(means_nut), bar_width,
# # #                 alpha=opacity, color='g',
# # #                 yerr=std_men, error_kw=error_config,
# # #                 label='Neutral')
# # # rects4 = ax.bar(index+ 3*bar_width, tuple(means_tot), bar_width,
...
# # # ax.set_title('Correlation between tweet corpus and stock prices')
# # # ax.set_xticks(index + bar_width / 2)
# # # ax.set_xticklabels((dateVal for dateVal in np.unique(date_split)))
# # # ax.legend()
